...
int main(int argc, char** argv) {
  ...
  options.add_options()
      ...
      ("inputpath", po::value<std::string>()->default_value("./"),
  ...
  std::string inpath = vm["inputpath"].as<std::string>();
  ...
  petuum::PSTableGroup::CreateTable(0, u_config);
  ...
  petuum::PSTableGroup::CreateTable(1, p_config);
  ...
  petuum::PSTableGroup::CreateTable(2, t_config);
  ...
  basepath << inpath << "/out" << num_workers;
  for (int i = 0; i < num_workers; i++) {
    threads[i] = std::thread(
        &tfals::Worker::run,
        std::unique_ptr<tfals::Worker>(new tfals::Worker(
            i, basepath.str(), rank, iterations, eval_rounds, 0, 1, 2)));
  }
