...
DEFINE_bool(cost_sensitive, false, "Whether to use const sensitive prediction");
...
DEFINE_int32(stats_table_id, 2, "Table with confusion matrix and accuracy metrics in PS.");
...
int main(int argc, char *argv[]) {
    ...
    table_config.table_info.row_capacity = softmax::kNumColumnsLossTable;
    ...
    table_config.table_info.row_type = kDenseRowFloatTypeID;
    table_config.table_info.table_staleness = 0;
    table_config.table_info.row_capacity = num_labels + 1;
    table_config.process_cache_capacity = num_labels + 1;
    table_config.oplog_capacity = num_labels + 1;
    petuum::PSTableGroup::CreateTable(FLAGS_stats_table_id, table_config);
    ...
    for (auto& thr : threads) {
        thr = std::thread(&softmax::Engine::Start, std::ref(mlr_engine));
    }
    for (auto& thr : threads) {
        thr.join();
    }
