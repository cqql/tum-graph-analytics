...
void Engine::ReadData() {
    ...
    } else if (read_format_ == "hdf5") {
		
        ReadHDF5Data(train_file, feature_dim_, num_train_data_,
                     &train_features_, &train_labels_, "train", feature_one_based_, label_one_based_);
        if (perform_test_) {
            LOG(INFO) << "Reading test file: " << FLAGS_test_file;
            ReadHDF5Data(FLAGS_test_file, feature_dim_,
                         num_test_data_, &test_features_, &test_labels_, "test", feature_one_based_, label_one_based_);
        }
    }
}

void Engine::ReadHDF5Features(const std::string& filename,
                              int32_t feature_dim, int32_t num_data, std::string datasetName,
    ...
    if( type_class == H5T_INTEGER ) {
		 LOG(INFO) << type_class << " INT";
	}
	else if( type_class == H5T_FLOAT ) {
		 LOG(INFO) << type_class << " FLOAT";
	}

    // Get order of datatype and print message if it's a little endian.
    //~ H5std_string order_string;
    //~ H5T_order_t order = intype.getOrder( order_string );
    //~ LOG(INFO) << order_string << endl;
    ...
    offset_out[0] = 0;
    offset_out[1] = 0;
    count_out[0]  = num_data;
    count_out[1]  = feature_dim;
    memspace.selectHyperslab(H5S_SELECT_SET, count_out, offset_out);

    //float features_out[num_data][feature_dim]; /* output buffer */
    float** features_out = new float*[num_data]; /* output buffer */
    for (int r = 0; r < num_data; r++)
    {
		features_out[r] = new float[feature_dim];
        for (int c = 0; c < feature_dim; c++)
        {
            features_out[r][c] = 0.0;
        }
    }
    ...
    dataset.read(features_out, PredType::NATIVE_FLOAT, memspace, dataspace);
    ...
    {		
        ...
        for (int c = 0; c < feature_dim; c++)
        {
			LOG(INFO) << ">>>>>> Reading col: " << c << " / " << feature_dim;
            temp[c] = features_out[r][c];
        }
        (*features)[r] = new petuum::ml::DenseFeature<float>(temp);
    }
...
void Engine::ReadHDF5Labels(const std::string& filename,
    ...
    //float labels_out[num_data]; /* output buffer */
    float* labels_out = new float[num_data]; /* output buffer */
    for (int r = 0; r < num_data; r++)
    {
        labels_out[r] = 0.0;
    }
    ...
    // Read data from hyperslab in the file into the hyperslab in
    // memory and display the data.
    dataset.read(labels_out, PredType::NATIVE_FLOAT, memspace, dataspace);
    ...
    for (int r = 0; r < num_data; r++)
    {
        (*labels)[r] = static_cast<int32_t>(labels_out[r]);
        if (label_one_based) {
            CHECK_LE(0, --(*labels)[r]) << "label is not one-based";
        }
    }
...
void Engine::ReadHDF5Data(const std::string& filename,
                          int32_t feature_dim, int32_t num_data,
                          std::vector<petuum::ml::AbstractFeature<float>*>* features,
                          std::vector<int32_t>* labels,
                          std::string datasetName,
    ...
    // Read features
    ReadHDF5Features(filename, feature_dim, num_data, datasetName, features);
	
    // Read labels
    ReadHDF5Labels(filename, feature_dim, num_data, labels, datasetName, label_one_based);
...
void Engine::Start() {
    ...
    solver_config.cost_sensitive = FLAGS_cost_sensitive;
