...
class Engine {
    ...
    private:  // functions
        ...
        void ReadHDF5Data(const std::string& filename,
                          ...
                          std::vector<int32_t>* labels, std::string datasetName,
                          bool feature_one_based = false, bool label_one_based = false);

        void ReadHDF5Features(const std::string& filename,
                              int32_t feature_dim, int32_t num_data, std::string datasetName,
                              std::vector<petuum::ml::AbstractFeature<float>*>* features);

        void ReadHDF5Labels(const std::string& filename,
                            int32_t feature_dim, int32_t num_data,
        ...
        void ComputeTestError(SGDSolver* solver,
        ...
        // This is distributed. This will reset test_workload_mgr.
        void ComputeStats(SGDSolver* solver,
                          petuum::ml::WorkloadManager* test_workload_mgr,
                          int32_t num_data_to_use, int32_t ith_eval);

        std::string ComputeAndPrintStats(SGDSolver* solver);

        // Compute online training error based on the first num_data_to_use. This
        // will reset workload_mgr.
        void ComputeTrainError(SGDSolver* solver,
    ...
    private:
        ...
        petuum::Table<float> stats_table_;
};
